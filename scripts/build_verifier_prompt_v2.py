"""Verifier v2 input: prompt + all 351 papers + the 210-claim ledger."""
...
DUMP = Path("/Users/abishek/personal/scispace-eval/data/dump")

claims = json.loads(
    re.search(r"```json\s*(\[.*?\])\s*```", (DUMP / "claims_v2_cea2bed8.md").read_text(), re.S).group(1)
)
evidence = (DUMP / "adjudication_pack_cea2bed8.md").read_text().split("---\n", 1)[1].strip()

PROMPT = r"""# Claim Verifier
...
w("# THE EVIDENCE")
w("")
w(f"Every source available to the report's writers. Ids `P1` onward. Some also carry an `E` id from an earlier run; ignore it.")
w("")
w("Sources with no abstract are listed with metadata only — treat them as unusable.")
w("")
w(evidence)

text = "\n".join(out)
dest = DUMP / "verifier_prompt_v2.md"
